...
- Doctor warns about cron jobs still marked as in-flight (stale `runningAtMs` from a killed gateway mid-run) so operators are not misled by a persistent `running` status.
